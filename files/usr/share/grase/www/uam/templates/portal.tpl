...
{/if}
    ...
         <button class="showLink visible-xs btn btn-lg btn-primary btn-block" style="max-width: 300px; margin: 0 auto"  onclick="$('#tos').toggleClass('hidden-xs')">{t}Show Terms and Conditions{/t}</button>
         <div id="tos" class="hidden-xs" style="height:8em;width:100%;border:1px solid #ccc;overflow:auto;margin:auto">
                <!-- Terms and Conditions -->
                {$tpl_termsandconditions}
         </div>
    </div>
    {/if}
...
{/if}
    ...
        <p class="error" id="errormessages"></p>
        ...
                <button class="btn btn-lg btn-primary btn-block" type="SUBMIT">Login</button>
                {if $nojs}
            <p>{t}You have disabled the secure javascript login method.{/t} <a href="?enablejs">{t}Click here to re-enable it{/t}</a></p>
            {/if}        </form>
        </div>
