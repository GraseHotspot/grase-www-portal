...
{
    global $NewSettings, $location, $pricemb, $pricetime, $support_name,
           $support_link, $website_link, $website_name, $locale, $mb_options,
           $time_options, $kbit_options, $DEMO_SITE;

    $location = $NewSettings->getSetting('locationName');
    if ($location == "") {
        $location = "Default";
    }
    $pricemb = $NewSettings->getSetting('priceMb');
    if ($pricemb == "") {
        $pricemb = 0.6;
    }
    $pricetime = $NewSettings->getSetting('priceMinute');
    if ($pricetime == "") {
        $pricetime = 0.1;
    }
    $support_name = $NewSettings->getSetting('supportContactName');
    if ($support_name == "") {
        $support_name = "Tim White";
    }
    $support_link = $NewSettings->getSetting('supportContactLink');
    if ($support_link == "") {
        $support_link = "http://grasehotspot.com/";
    }

    $website_link = $NewSettings->getSetting('websiteLink');
    if ($website_link == "") {
        $website_link = "http://grasehotspot.org/";
    }
    $website_name = $NewSettings->getSetting('websiteName');
    if ($website_name == "") {
        $website_name = "GRASE Hotspot Project";
    }

    $mb_options = $NewSettings->getSetting('mbOptions');
    if ($mb_options == "") {
        $mb_options = "10 50 100 250 500 1024 2048 4096 102400";
    }
    $time_options = $NewSettings->getSetting('timeOptions');
    if ($time_options == "") {
        $time_options = "5 10 20 30 45 60 90 120 180 240 600 6000";
    }
    $kbit_options = $NewSettings->getSetting('kbitOptions');
    if ($kbit_options == "") {
        $kbit_options = "64 128 256 512 1024 1536 2048 4096 8192";
    }

    $locale = $NewSettings->getSetting('locale');
    if ($locale == '') {
        $locale = "en_AU";
    }
