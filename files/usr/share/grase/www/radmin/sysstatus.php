...
// PID File
    ...
    // Ping host to see if up
        ...
        if ($return == 1) {
            output_status($proc, 0, ''); //Host down
        }        if ($return == 2) {
            output_status($proc, 0, '');// Error (-1)
        }        if ($return == 0) {
            output_status($proc, 1, $avg_time, 'ping');// Host up
        }
...
function output_status($proc, $status, $time, $type = 'proc')
    ...
    if ($status ===1) {
        $status = 'running';//$status = "<img src='images/green.png'/>Running";
    }    if ($status === 0) {
        $status = 'stopped';//$status = "<img src='images/red.png'/>Not Running";
    }    if ($status === -1) {
