...
case 'initialpage':
    ...
    {
        $valid = true; // Assume form is valid
        // We have a form submitted, check it's valid and move to the next step
        
        // Check voucher is valid
        if(!in_array($_POST['voucherselected'], $valid_vouchers)) $valid=false;
        
        // Check payment gateway is valid
        if(!array_key_exists($_POST['gatewayselected'], $paymentgateways)) $valid=false;
...
case 'paymentpage':
    ...
    if(!is_file('paymentgateways/'.$paymentgateways[$_SESSION['selectedpaymentgateway']]['pluginfile'])) die("Invalid plugin"); // TODO Clean up and make error detection lots lots better
