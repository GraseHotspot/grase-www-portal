...
<tr><td colspan='2' style='color: red'>If you really want to delete this machine account, please type exactly "Y<span style="display:none;">PLEASE DON'T COPY AND PASTE</span>es, I want to delete this use<span style="display:none;">PLEASE DON'T COPY AND PASTE</span>r" in the below box then click "Delete User"</td><tr/>
	<tr><td></td><td><form method='post'><input size='30' type="text" name="DeleteUser"/><button class="negative" type="submit" name="deleteusersubmit" value="Delete User"><img src="/images/icons/cross.png" alt=""/>Delete User</button></form></td></tr>
</table>
{else}
<div class="errorPage" style="display: {if $error}block;{else}none;{/if}"> <span id="errorMessage">{$error}</span> </div>
<table>
	<tr><td>Username</td><td>{$user.Username}</td></tr>
	<tr><td>Password</td><td><form method='post'> <input type="text" name="Password"/ value=''/><button type="submit" name="changepasswordsubmit" value="Change Password"><img src="/images/icons/textfield_key.png" alt=""/>Change Password</button></form></td></tr>
	...
	<tr><td>Expiry (Automatic)<a class="helpbutton" onclick="ShowContent('helpbox','Expiry is based on the Group.<br\/>1 Month for visitors<br\/>3 Months for students<br\/>6 Months for staff and ministry');" ><img src="/images/icons/help.png" alt=""/></a></td><td>{$user.FormatExpiration}
