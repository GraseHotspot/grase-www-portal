...
<div class="errorPage" style="display: block;"> <span id="errorMessage">Machine Account Locked (Only Admin can edit){if $error}<br/>{$error}{/if}</span> </div>
<table>
...
<div class="noticePage" style="display: {if $notice}block;{else}none;{/if}"> <span id="noticeMessage">{$notice}</span> </div>
<form method='post'>
<table>
	<tr><td>Username</td><td>{$user.Username}</td></tr>
