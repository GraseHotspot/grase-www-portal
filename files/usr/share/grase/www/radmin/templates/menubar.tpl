<div id="menubar" class=" ui-widget-content">
...
{foreach from=$MenuItems key=menuitemid item=menuitem name=toploop}
    <li class="
        {if $menuitemid == "logout"}negative{/if}">
            <span class="collapse"><span class="ui-icon ui-icon-circle-triangle-s">-</span></span>
            <span class="expand"><span class="ui-icon ui-icon-circle-triangle-w">+</span></span>            
        
            <a id="{$menuitemid}" 
                href="{$menuitem.href}" class="{if $activepage == $menuitemid}ui-state-active ui-state-highlight{/if} topmenu" >{$menuitem.label}</a>
        {if $menuitem.submenu}
